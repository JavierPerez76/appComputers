...
def main():
    try:
        ...
        if uploaded_file:
            ...
            # Analizar el texto extraído del PDF con CLU
            language_client = ConversationAnalysisClient(
                ls_prediction_endpoint, AzureKeyCredential(ls_prediction_key)
            )
            ...
            # Extraer características del texto y guardarlas en MongoDB
            entities = result["result"]["prediction"]["entities"]
            # Aquí puedes agregar el código para insertar los datos extraídos a la base de datos MongoDB
            new_computer = {
                "entities": {entity["category"]: entity["text"] for entity in entities}
            }
            collection.insert_one(new_computer)
            st.success("El pedido ha sido realizado con éxito.")
